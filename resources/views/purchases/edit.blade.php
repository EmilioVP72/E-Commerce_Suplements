        ...
        <div class="card-body">
            <form action="{{ route('purchases.update', $purchase->id_purchase) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('id_user') is-invalid @enderror" id="id_user" name="id_user" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('id_user', $purchase->id_user) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('id_user')
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('sail_date') is-invalid @enderror" id="sail_date" name="sail_date" value="{{ old('sail_date', $purchase->sail_date) }}" required>
                    @error('sail_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
