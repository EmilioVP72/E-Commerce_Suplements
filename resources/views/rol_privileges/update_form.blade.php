...
@section('content')
<div class="container">
    <h1>Actualizar Rol-Privilegio</h1>

    <div class="card shadow-sm">
        <div class="card-body">
            <form id="updateRolPrivilegeForm">
                @csrf
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('rol_privileges.index') }}" class="btn btn-secondary me-2">Cancelar</a>
...
document.getElementById('updateRolPrivilegeForm').addEventListener('submit', async function(event) {
    event.preventDefault();

    const form = event.target;
    const formData = new FormData(form);
    const url = `{{ url('/api/rol-privileges') }}/${rolPrivilegeId}`;
