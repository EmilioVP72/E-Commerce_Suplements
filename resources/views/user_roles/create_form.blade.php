...
document.getElementById('createUserRoleForm').addEventListener('submit', async function(event) {
    event.preventDefault();

    const form = event.target;
    const formData = new FormData(form);
    const url = "{{ url('/api/user-roles/user-roles/assign') }}";
    ...
    try {
        ...
        if (response.ok) {
            alert(result.message || 'Rol asignado con éxito');
