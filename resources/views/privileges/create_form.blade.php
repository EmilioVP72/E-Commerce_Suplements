        ...
        <div class="card-body">
            <form id="createPrivilegeForm">
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <button type="submit" class="btn btn-primary">Guardar Privilegio</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
document.getElementById('createPrivilegeForm').addEventListener('submit', async function(event) {
    event.preventDefault();

    const form = event.target;
    const formData = new FormData(form);
    const url = "{{ url('/api/privileges/StorePrivilege') }}";
    ...
    try {
        ...
        if (response.ok) {
            alert(result.message || 'Privilegio creado con éxito');
