...
@section('content')
<div class="container">
    <h1 class="my-4">Crear Nuevo Detalle de Transacción</h1>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('id_transaction') is-invalid @enderror" id="id_transaction" name="id_transaction" required>
                        ...
                        @foreach($transactions as $transaction)
                            <option value="{{ $transaction->id_transaction }}" {{ old('id_transaction') == $transaction->id_transaction ? 'selected' : '' }}>Transacción #{{ $transaction->id_transaction }}</option>
                        @endforeach
                    </select>
